...
#            pygame.draw.line( pSurfDest, ( 200, 200, 200), (w*self.cBullW, 0), (w*self.cBullW, self.cBullH*self.HEIGHT))
    ...
    def shiftDownOneRow( self ):
        ...
        return True
         ...
         #-----------------------------------------
    def shiftDownRows( self, nb ):
        for i in range( 0, nb):
            if self.shiftDownOneRow() == False:
                return False
            else:
                self.randomFirstRow()
    ...
    def shiftDownRange( self ):
        return  self.shiftDownRows( (self.MAX_BULLET_TYPE+1) - len( self.cLifeType ))
    ...
    def destroyAnimation( self ):
        ...
        #-----------------------------------------
    def gameOverAnimation( self, pSurfDest ):

        self.cGameFinish -= 1

        pSurfDest.blit( self.cGameOver, (100, 100))


         #-----------------------------------------
    def gameWinAnimation( self, pSurfDest ):

        self.cGameFinish -= 1

   #     self.cRect = pygame.Rect( (100,100), )
        pSurfDest.blit( self.cYouWin, (100, 100))
       


def CallbackReset():
    World.sTheWorld.resetGame()
